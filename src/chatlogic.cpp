...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                // TODO: I think this is a case of: R. 30 : Take smart pointers as parameters only to explicitly express lifetime semantics
                //       should we not make every instance of GraphNode const in the other nodes?
